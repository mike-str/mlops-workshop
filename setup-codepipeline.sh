...
IAM_USERNAME=$(aws sts get-caller-identity --query Arn --output text | cut -d '/' -f 3)
...
AWS_REGION=$(echo $GITHUB_CONNECTION_ARN | cut -d ':' -f 4)
...
echo "Please approve the GitHub connection in your browser: https://$AWS_REGION.console.aws.amazon.com/codesuite/settings/connections/$GITHUB_CONNECTION_ARN"
